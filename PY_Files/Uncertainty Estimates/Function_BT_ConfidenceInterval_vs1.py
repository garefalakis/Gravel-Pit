...
# from Load_ProximalDistalData_ApLuTh import * ### will be loaded below (if needed)
...
### OLD SLOW CODE, SAME AS ABOVE BUT SLOWER!    
        # for i in range(iterations):
            
        #     resample = np.random.choice(data[count], size=len(data[count]), replace=True)
        #     ### data_resampled.append(resample)                                 ### Store resampled data (not necessary! Uses memory)
        #     perc_resampled[count].append(np.percentile(resample, perc))         ### Calculate the percentile on simulated values
        #     avg_resampled[count].append(np.average(resample))                   ### Calculate the average on simulated values
            
        # count += 1
        
    # ### Create lists to store standard deviation of resampled percentiles:
    # std_perc_resampled = []
    # std_avg_resampled = []
    
    # ### Calculate the standard deviation of individual resampled percentiles:
    # for i in range(n):
        
    #     std_perc_resampled.append(np.std(perc_resampled[i]))                    ### Calculate the Std Dev of the Percentiles
    #     std_avg_resampled.append(np.std(avg_resampled[i]))                      ### Calculate the Std Dev of the BT Average
### END OF OLD SLOW CODE, SAME AS ABOVE BUT SLOWER!   
...
### Then Call the Function as: e.g. for D50
### lower_CI, upper_CIT = boostrap_CI(ThLS_Di, 50, 1000)
