...
# TestD16 = percentile_to_find(SievePassedReverse, SieveBinsReverse, 16)

### ALTERNATIVE: perc_16 = np.interp(16, GAD_LVA_sumfreq*100, bins_photo_hand[0:len(GAD_LVA_sumfreq)])
### where GAD_LVA_sumfreq i.e. cumsum frequency of counts in bins; bins_photo_hand i.e. sieve bins ### see Call_Plot_HandPhotoSieve_Percentiles_vs5
...
# testrange1 = (2,4,1,1,2,4,2,0.5,0.6,1,2)
# testrange1 = np.asarray(testrange1)
# testrange2 = np.arange(0,20,1)
# testrange3 = np.arange(0,2,0.1)

# data = [testrange1, testrange2, testrange3]
# mm_smaller = 2.0
...
# min_cax = []
# for i in range(8):
#     calcavg = np.average(hand_all_A_axes[i])
#     averages.append(calcavg)
#     calcmaxb = max(hand_all_B_axes[i])
#     max_bax.append(calcmaxb)
#     calcminaax = min(hand_all_A_axes[i])
#     min_aax.append(calcminaax)
#     calcminbax = min(hand_all_B_axes[i])
#     min_bax.append(calcminbax)
#     calcmincax = min(hand_all_C_axes[i])
#     min_cax.append(calcmincax)
