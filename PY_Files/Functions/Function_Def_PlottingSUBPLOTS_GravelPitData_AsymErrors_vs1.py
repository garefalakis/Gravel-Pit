...
# sys.path.append("D:\\OneDrive - Universitaet Bern\\07_PhD\\06_Programming\\Python Scripts\\Python_Functions") ### laptop
...
def subplot_percentile_per_set_hand_abc_photo_asymerror(data1, data2, data1_SD, data2_SD, data1_name, data2_name, plotname, offset, colors_dots):
    ...
    # plotlim1, plotlim2 = ceil_xyaxis_noarray(hand_abc, hand_abc_SD, photo, photo_SD)
    ...
    for j in range(4):
        for i in range(4):      
            axs[j].errorbar(0, 0, 0, 0, ls='', elinewidth=0.7, capsize=1.5, capthick=0.4, ms=1.5,
                                    color=colors_dots[i], fmt=style_dot[i], label=dot_label[i])
